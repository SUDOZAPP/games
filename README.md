...
| 程式 | 內容 | 檔案大小 |
|---|---|---:|
| `Balls2018-v2.5-Relaxed-Orchestra.exe` | 太空球體射擊與 Relaxed Orchestra 音樂體驗 | 約 0.7 MB |
| `Lucky-Bahasa-Slot-v1.5.9.exe` | 印尼語字母、單字、會話與拼字拉霸 | 約 8.7 MB |
| `Lucky-Kana-Slot-v1.10.9.14.exe` | 日語五十音、平假名、片假名與發音練習 | 約 13.6 MB |
| `Lucky-Turkish-Slot-v1.8.0.exe` | 土耳其語字母、單字與 Apple 風格會話練習 | 約 6.8 MB |
| `lucky-word-slots-v3.3.1.exe` | 適合課堂使用的幸運單字拉霸 | 約 2.6 MB |
| `pachinko-moon-maiden-v2.3.25.11.exe` | 紅月巫女 Pachinko 互動遊戲 | 約 15.1 MB |
| `SQFT-Formula-Slot-v0.8.11.6.exe` | SQFT 公式、符號與概念學習拉霸 | 約 5.1 MB |
| `SUOU-Element-Periodic-Table-Slot-v1.11.4.exe` | 元素週期表問答與英文發音練習 | 約 9.1 MB |
| `Terra-S077.40.7.6.22.exe` | SUOU FieldSense Terra 多語言互動體驗 | 約 9.1 MB |
...
1. 在 GitHub 中選擇要下載的 `.exe` 檔案。
2. 按下 **Download raw file**，或從 Releases 頁面下載。
3. 下載完成後直接雙擊執行。
4. 首次播放音效前，請先點擊或觸碰遊戲畫面。

每個 EXE 會將內嵌遊戲載入到 Microsoft Edge 的獨立應用程式視窗。學習進度與設定由瀏覽器保存在本機裝置上。

### Windows 安全提醒

這些 EXE 尚未使用商業程式碼簽章憑證簽署，因此 Windows SmartScreen 可能在第一次執行時顯示提醒。請只從本儲存庫的正式頁面下載檔案，並在確認檔名與來源後執行。

### 疑難排解

- **程式沒有開啟：** 確認 Microsoft Edge 已安裝並更新至最新版。
- **沒有聲音：** 先點擊畫面，再檢查 Windows、Edge 與應用程式內的音量設定。
- **英文或日文語音無法播放：** 確認 Windows 已安裝對應的系統語音；部分遊戲會使用瀏覽器語音作為備援。
- **攝影機模式沒有畫面：** 在 Edge 的權限提示中允許攝影機存取。
- **ICON 沒有立即更新：** Windows 可能暫存舊圖示；重新整理檔案總管或重新登入即可。

### 隱私

遊戲不需要建立帳號。進度、偏好與遊戲狀態主要保存在使用者的本機裝置。攝影機功能只在使用者主動選擇並授權後啟用。
